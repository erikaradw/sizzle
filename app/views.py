...
@login_required
def resep_tersimpan(request):
    user = request.user
    user_resep = UserResep.objects.filter(user=user).select_related('resep')
    resep_tersimpan_list = [user_resep.resep for user_resep in user_resep]

    context = {
        'user': user,
        'resep_tersimpan_list': resep_tersimpan_list
    }
    return render(request, 'halamanku.html', context)
...
def halamanku(request):
    user = request.user
    resep_tersimpan_list = UserResep.objects.filter(user=user).select_related('resep')

    for ur in resep_tersimpan_list:
        print(f"{ur.resep.nama_resep}")

    context = {
        'user': user,
        'resep_tersimpan_list': [ur.resep for ur in resep_tersimpan_list]
    }
    return render(request, 'halamanku.html', context)
